...
public class GoNativeActivity extends NativeActivity {
	public static GoNativeActivity goNativeActivity;
	...
	private static BlockingQueue<Boolean> initQueue = new LinkedBlockingQueue<>();
    ...
    private static final int REQUEST_CODE_CONTACTS = 420; // 1

    private static BlockingQueue<Boolean> contactPermResp = new LinkedBlockingQueue<>();
    ...
    public void onRequestPermissionsResult(int requestCode, String[] permissions, int[] grantResults) {
        if (requestCode == REQUEST_CODE_CONTACTS) {
            try {
                contactPermResp.put(allPermissionsGranted(grantResults));
            } catch (InterruptedException e) {
                Thread.currentThread().interrupt();
            }
        } else {
    ...
    public static IconCompat notifyIcon;

    public static void setNotifyIcon(byte[] icon) {
         Bitmap iconBitmap = BitmapFactory.decodeByteArray(icon, 0, icon.length);
         notifyIcon = IconCompat.createWithBitmap(iconBitmap);
    }
    ...
    public void showNotify() {
        ...
        NotificationCompat.Builder builder = new NotificationCompat.Builder(this, "my_id_1")
                .setSmallIcon(notifyIcon) // Иконка уведомления
                .setContentTitle("Title 1") // Заголовок уведомления
                .setContentText("Notify text") // Текст уведомления
    ...
    private void createNotificationChannel(Context context) {
        if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.O) {
            ...
            NotificationChannel channel = new NotificationChannel("notify_chan_01", name, importance);
